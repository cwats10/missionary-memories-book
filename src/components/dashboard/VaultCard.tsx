import { VaultWithRole } from '@/hooks/useVaults';
...
export function VaultCard({ vault, onDelete, onView }: VaultCardProps) {
  ...
  return (
    <Card className="group hover:shadow-md transition-shadow">
      ...
      <CardContent>
        {vault.description && (
          <p className="text-sm text-muted-foreground mb-4 line-clamp-2">{vault.description}</p>
        )}
        {vault.occasion && (
          <p className="text-xs text-muted-foreground mb-4">Occasion: {vault.occasion}</p>
        )}
